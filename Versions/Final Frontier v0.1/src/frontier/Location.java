...
public class Location {
	...
	public void displayLocation(Panel terminal) {
		...
		terminal.write("Welcome to " + name + "!", 5, 3);
		terminal.write("Star exports: " + exports.name(), 5, 4);
		terminal.write("Star imports: " + imports.name(), 5, 5);
		terminal.write("Player dough: $" + Main.playerDough, 5, 7);
		...
		for (int i=0; i<10; i++) {
			if (i<5) {
				if (Main.playerInventory[i] > 0) {
					terminal.write(Main.commodityList[i].name(), 5, 16 + i);
					terminal.write("" + Main.playerInventory[i], 31, 16 + i);
				...
				if (Main.playerInventory[i] > 0) {
					...
					terminal.write("" + Main.playerInventory[i], 68, 11 + i);
