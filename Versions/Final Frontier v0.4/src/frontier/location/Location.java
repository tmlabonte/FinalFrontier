...
public class Location {
	...
	//Returns distance between two locations (depends on travelSpeed of current ship)
	public int distanceTo(Location destination) {
		return (int) (((10 - Player.currentShip().travelSpeedDouble()) / 5) * (Math.sqrt(Math.pow(this.xPos() - destination.xPos(), 2) + Math.pow(this.yPos() - destination.yPos(), 2))));
	}
	...
	//Replenishes all location inventories based on distance traveled 
	//Every 10ly, increment export by 2, import by 0, everything else by 1
	public static void replenish(int distance) {
		for (int d=0; d < distance; d++) {
			Player.changeDistanceTraveled(1);
			if (Player.distanceTraveled() % 10 == 0) {
				for (Location location : Main.locationList) {
					for (int i=0; i < Main.commodityList.length; i++) {
						if (Main.commodityList[i] == location.exports()) {
							location.locationInventory()[i] += 2;
						}
						else {
							location.locationInventory()[i]++;
						}
					}
				}
			}
