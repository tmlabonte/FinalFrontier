...
public class CantinaScreen extends Screen {
	...
	public Screen respondToInput(MouseEvent m) {
		...
		if (m.getID() == MouseEvent.MOUSE_CLICKED && m.getX() >= 380 && m.getX() <= 700 && m.getY() >= 80 && m.getY() <= 200) {
			...
			//If there is a crew member in that spot...
				...
				if (Player.playerCash() >= Main.crewList.get(index).signingFee() && IntStream.of(Player.playerCrew()).sum() < Player.currentShip().maxCrew()) {
					...
					Player.currentShip().multiplyStat(Main.crewList.get(index).specialty().toString(), Main.crewList.get(index).multiplier());
				}
		...
		//If the player clicks on a crew member in the left side of their inventory...
			...
			if (index != -1) {
				...
				//Negate crew multiplier
				Player.currentShip().multiplyStat(Main.crewList.get(index).specialty().toString(), (1 / (Main.crewList.get(index).multiplier())));
			}
	...
	public void displayOutput(Panel terminal) {
		...
		//Displays crew names, specialties, multipliers, fees, and salaries
		int currentLine = 3;
