...
public class ShipyardsScreen extends Screen {
	...
	public ShipyardsScreen() {
		...
		hullNeeded = Player.currentShip().maxHull() - Player.currentShip().currentHull();
	}
	...
	public Screen respondToInput(MouseEvent m) {
		...
		if (m.getID() == MouseEvent.MOUSE_CLICKED && m.getX() >= 460 && m.getX() <= 580 && m.getY() >= 65 && m.getY() <= 75) {
			...
			if (Player.playerCash() >= 2 * foodNeeded) {
				Player.currentShip().changeCurrentFood(foodNeeded);
				Player.changePlayerCash(2 * foodNeeded);
			}
			else {
				int foodPurchasable = Player.playerCash() / 2;
				Player.currentShip().changeCurrentFood(foodPurchasable);
		...
		//If the player clicks "Replenish Fuel"
			...
			if (Player.playerCash() >= 2 * fuelNeeded) {
				Player.currentShip().changeCurrentFuel(fuelNeeded);
				Player.changePlayerCash(-2 * fuelNeeded);
			}
			else {
				int fuelPurchasable = Player.playerCash() / 2;
				Player.currentShip().changeCurrentFuel(fuelPurchasable);
				Player.changePlayerCash(-2 * fuelPurchasable);
			}

			return new ShipyardsScreen();
		}
		//If the player clicks "Repair Hull"
		else if (m.getID() == MouseEvent.MOUSE_CLICKED && m.getX() >= 460 && m.getX() <= 580 && m.getY() >= 95 && m.getY() <= 105) {
			//Repair as much hull as able
			if (Player.playerCash() >= 2 * hullNeeded) {
				Player.currentShip().changeCurrentHull(hullNeeded);
				Player.changePlayerCash(-2 * hullNeeded);
			}
			else {
				int hullPurchasable = Player.playerCash() / 2;
				Player.currentShip().changeCurrentHull(hullPurchasable);
				Player.changePlayerCash(-2 * hullPurchasable);
			}
	...
	public void displayOutput(Panel terminal) {
		...
		//Display options to repair hull
		terminal.write("Repair Hull", 50, 4);
		terminal.write("$" + 2 * hullNeeded, 67, 4);
