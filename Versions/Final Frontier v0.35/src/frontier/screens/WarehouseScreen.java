...
public class WarehouseScreen extends Screen {
	...
	public Screen respondToInput(MouseEvent m) {		
		...
		if (m.getID() == MouseEvent.MOUSE_CLICKED && m.getX() >= 380 && m.getX() <= 700 && m.getY() >= 65 && m.getY() <= 215) {
			...
			if (Location.currentInventory()[indexInArray] > 0 && Player.playerCash() >= Main.commodityList[indexInArray].basePrice() && IntStream.of(Player.playerInventory()).sum() < Player.currentShip().maxCommodities()) {
